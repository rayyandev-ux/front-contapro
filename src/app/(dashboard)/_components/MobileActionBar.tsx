  ...
  return (
    <nav className="md:hidden fixed bottom-3 left-1/2 z-30 w-[calc(100%-1rem)] max-w-md -translate-x-1/2 rounded-2xl border border-border bg-card/95 p-2 shadow-lg backdrop-blur">
      <ul className="grid grid-cols-5">
        ...
        <li>
          <Link href="/history" onClick={() => onNavigate?.()} className={`${itemBase} ${isActive("/history") ? "text-primary" : "text-muted-foreground"}`}>
        ...
        <li>
          <Link href="/budget" onClick={() => onNavigate?.()} className={`${itemBase} ${isActive("/budget") ? "text-primary" : "text-muted-foreground"}`}>
