...
export default function PaymentMethodBudgetView({
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar presupuesto de este método de pago?")) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-zinc-100">Presupuesto por Método de Pago</h2>
          <p className="text-zinc-400">Controla el gasto por tarjeta o cuenta en {monthLabel}</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="bg-zinc-900/50 border-zinc-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-zinc-100">{formatCurrency(generalAmount)}</div>
        ...
        <Card className="bg-zinc-900/50 border-zinc-800">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-zinc-100">{formatCurrency(totalAssigned)}</div>
            <p className="text-xs text-zinc-500">
              {pmStatuses.filter(s => s.budget).length} métodos configurados
            </p>
          </CardContent>
        </Card>
        <Card className="bg-zinc-900/50 border-zinc-800">
          ...
          <CardContent>
            ...
            <p className="text-xs text-zinc-500">Disponible para nuevos métodos</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-zinc-900/50 border-zinc-800">
        <CardHeader>
          <CardTitle className="text-zinc-100">Estado de Métodos</CardTitle>
        </CardHeader>
        <CardContent>
          {pmStatuses.filter(s => s.budget).length === 0 ? (
            <div className="text-center py-10 text-zinc-500">
              No hay presupuestos configurados para métodos de pago.
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {pmStatuses.filter(s => s.budget).map(item => {
                ...
                return (
                  <div key={item.paymentMethodId} className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-4 relative group">
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-400 hover:text-red-400" onClick={() => handleDelete(item.paymentMethodId)}>
        ...
        <DialogContent className="sm:max-w-[425px] bg-zinc-950 border-zinc-800 text-zinc-100">
          ...
          <form action={handleSave} className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="method" className="text-zinc-300">Método de Pago</Label>
              {editingItem ? (
